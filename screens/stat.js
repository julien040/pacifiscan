  ...
  useEffect(() => {
    (async () => {
      ...
      let array = [
        getCaddyCount(c),
        getCountCo2(c),
        differentTypeDiscovered(data),
        getTimeCount(data),
        getWeightCount(data),
        mostScannedType(data),
      ];
      setData(array);
    })();
  }, []);
  return (
    ...
    >
      ...
      <Flex
        ...
        flex={1}
      >
        <Heading width="100%" marginTop={4} marginBottom={2}>
          Statistiques
        </Heading>
        ...
        <SingleItem
          data={Data[3] + " ans"}
          text="Temps de décomposition cumulé"
        />
        <SingleItem data={Data[4] + " kg"} text="Poids cumulé des déchets" />
        <SingleItem
          smaller
          data={associationApi[Data[5]]}
          text="Type de déchets le plus scanné"
        />
      </Flex>
      <Text
        onPress={() => {
          Share.share({
            message: `Grâce à l'application Pacifiscan, j'ai ramassé ${Data[4]} kg de déchets`,
          });
        }}
        textAlign="right"
        color="brand.iris80"
...
const SingleItem = ({ data, text, smaller }) => {
  return (
    <Flex
      ...
      width="49%"
    >
      <Heading fontSize={smaller ? 16 : 26} marginTop={2}>
        {data}
      </Heading>
      <Text marginTop={2} color="brand.iris80">
        {text}
      </Text>
    </Flex>
  );
